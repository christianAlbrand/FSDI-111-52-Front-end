...
def task_detail(pk):
    url = "%s/%s" % (BACKEND_URL, pk)       # this formats a string
...
def edit_task(pk):
    task_data = flask_req.form                      # the form attribute in flask's request object is a dictionary
    url = "%s/%s" % (BACKEND_URL, pk)
...
def get_delete_form(pk):                            # this will be a confirmation screen
    url = "%s/%s" % (BACKEND_URL, pk)
    response = requests.get(url)
    if response.status_code == 200:
        single_task = response.json().get("task")
